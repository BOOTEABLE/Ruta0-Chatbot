...
const configurarBaseDeDatos = async () => {
    try {
        ...
        // 👇 AJUSTE: agregamos 'horario' (que etl.js ya insertaba y esta tabla
        // no tenía -> rompía el ETL), 'confianza' (documentado en harness.md
        // pero nunca creado) y 'actualizado_en' (para saber qué tan vieja
        // está cada fila y poder detectar datos desactualizados).
        await pool.query(`
        ...
        await pool.query(`
            ...
            ('Café de la Vaca Centro', 'Cafetería', '$$', 'Excelente cafetería tradicional en el centro.', -0.2201, -78.5123, ST_SetSRID(ST_MakePoint(-78.5123, -0.2201), 4326), 'Horario no disponible', 100),
            ('Parque La Carolina', 'Parques', 'Gratis', 'El parque más grande del centro-norte.', -0.1839, -78.4831, ST_SetSRID(ST_MakePoint(-78.4831, -0.1839), 4326), 'Horario no disponible', 100),
            ('Yaku Museo del Agua', 'Museos', '$', 'Museo con excelentes vistas de la ciudad.', -0.2232, -78.5186, ST_SetSRID(ST_MakePoint(-78.5186, -0.2232), 4326), 'Horario no disponible', 100);
        `);
